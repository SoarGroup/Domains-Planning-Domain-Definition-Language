...
//                constraints? structureDef*)
      -> ^(DOMAIN domainName requireDef? typesDef?
                constantsDef? predicatesDef? structureDef*)
    ;
...
typedNameList
    : NAME+ -> ^(NAME NO_TYPE)+
    | singleTypeNameList+ NAME* -> singleTypeNameList+ ^(NAME NO_TYPE)*
    ;
...
typedVariableList
    : var+ -> ^(var NO_TYPE)+
    | singleTypeVarList+ var* -> singleTypeVarList+ ^(var NO_TYPE)*
    ;
